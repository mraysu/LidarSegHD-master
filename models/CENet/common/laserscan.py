...
class LaserScan:
    ...
    def __init__(self, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0,DA=False,flip_sign=False,rot=False,drop_points=False,percentLabels=None):
    ...
    def open_scan(self, filename):
        ...
        #===========================perform binary masking==============================
        if self.percentLabels is not None:
            numPoints = len(scan)
            self.binmask = np.ones(numPoints, dtype=bool)
            self.binmask[:int(numPoints * (1-self.percentLabels))] = False  #we want 1-percentLabels% of the points
            np.random.shuffle(self.binmask)
            scan = scan[self.binmask,...]
        #===========================perform binary masking=============================
...
class SemLaserScan(LaserScan):
    ...
    def __init__(self, sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, max_classes=300,DA=False,flip_sign=False,rot=False,drop_points=False,percentLabels=None):
        super(SemLaserScan, self).__init__(project, H, W, fov_up, fov_down,DA=DA,flip_sign=flip_sign,rot=rot,drop_points=drop_points, percentLabels=percentLabels)
        self.reset()
    ...
    def open_label(self, filename):
        ...
        #===========================perform binary masking==============================
        if self.percentLabels is not None:
            if self.binmask is None:
                raise RuntimeError("Defined percentLabels but no mask exists")
            label = label[self.binmask,...]
        #===========================perform binary masking=============================
